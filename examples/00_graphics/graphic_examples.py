...
fig1.set_axes(axis(0,[-4,4]), axis(1,[-4,4])) # (axis_id,[range_of_values_on_this_axis])
...
fig2.draw_circle([1,1],0.5,[Color([255,155,5]),Color([255,155,5,100])]) # drawing a circle at (1,1) of radius 0.5 with a custom RGB color
fig2.draw_ring([3,1],[0.5,1.0],[Color.red(),Color.green(0.2)]) # drawing a ring at (1,1) of radius [4,6] with a predefined red color

ctc_wrapper=SepWrapper(IntervalVector([[-1,1],[-1,1]]))
p=pave(IntervalVector([[-2,2],[-2,2]]),ctc_wrapper,0.1)
fig1.draw_paving(p)
