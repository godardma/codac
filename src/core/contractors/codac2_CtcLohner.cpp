...
const IntervalVector &LohnerAlgorithm::integrate(unsigned int steps, double H) {
  ...
  for (unsigned int i = 0; i < steps; ++i) {
    ...
    for (int j = 0; j < contractions; ++j) {
      ...
      u_hat1 = u_hat + h * direction * f->eval(u_hat).mid() + m1;
      u1 = u_hat1 + B1 * r1;
      if (j < contractions - 1) {
...
IntervalVector LohnerAlgorithm::globalEnclosure(const IntervalVector &initialGuess, double dir) {
  ...
  for (unsigned int i = 0; i < 30; ++i) {
    ...
    cout << "Global enclosure iteration " << i << ": " << u_1 << endl;
    cout << "Global enclosure iteration " << i << " (initial guess): " << u_0 << endl<< endl;
...
void CtcLohner::contract(codac2::SlicedTube<IntervalVector> &tube, TimePropag t_propa) {
  ...
  if ((t_propa & TimePropag::FWD) != static_cast<TimePropag>(0)) {
    for (int j = 0; j < dim; ++j) {
      input_gate[j] = tube[j].first_slice()->input_gate();
    }
    LohnerAlgorithm lo(&m_f, 0.1, true, input_gate, contractions, eps);
    // Forward loop
    // replace for (int i = 0; i < tube.nb_slices(); ++i) with iterator
    for (auto it = tube[0].tdomain()->begin(); it != tube[0].tdomain()->end(); ++it) {
      h = (*it).diam();
      for (int j = 0; j < dim; ++j) {
        output_gate[j] = tube[j].slice(it)->output_gate();
        slice[j] = tube[j].slice(it)->codomain();
      }
      lo.integrate(1, h);
      lo.contractStep(output_gate);
      for (int j = 0; j < dim; ++j)
        tube[j].slice(it)->set(slice[j] & lo.getGlobalEnclosure()[j]);  // set_envelope equivalent in codac2 ?
    }
  ...
  if ((t_propa & TimePropag::BWD) != static_cast<TimePropag>(0)) {
    ...
    for (auto it = tube[0].tdomain()->begin(); it != tube[0].tdomain()->end(); ++it) {
      h = (*it).diam();
      for (int j = 0; j < dim; ++j) {
        output_gate[j] = tube[j].slice(it)->input_gate();
        slice[j] = tube[j].slice(it)->codomain();
      }
      lo2.integrate(1, h);
      lo2.contractStep(output_gate);
      for (int j = 0; j < dim; ++j)
        tube[j].slice(it)->set(slice[j] & lo2.getGlobalEnclosure()[j]);
    }
